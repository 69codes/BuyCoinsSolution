...
# empty list to which the numbers would be appended into
num = []
def IsProthPrime(p):
    for i in range(10):
        # in python, "%" is a modulo operator, which returns the remainder
        num.append((((i)**((p - 1)/2)) + 1) % p) 
    if 0 in num:
    #if 0 is the remainder, then there was a complete division
